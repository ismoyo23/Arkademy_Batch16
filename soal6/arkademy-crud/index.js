...
app.get('/',(req, res) => {
  let sql = "SELECT product.*, chasier.*, category.* FROM product INNER JOIN chasier ON chasier.id_chasier = product.id_chasier INNER JOIN category ON category.id_category = product.id_category ORDER BY product.id ASC";
...
app.get('/save/:menu/:price/:name_category/:name_chasier',(req, res) => {
  let sql = "INSERT INTO product(name, price, id_category, id_chasier) VALUES ('"+req.params.menu+"', '"+req.params.price+"', '"+req.params.name_category+"', '"+req.params.name_chasier+"')";
  let query = conn.query(sql, (err, results) => {
    if(err) throw err;
    res.redirect('/');
  });
